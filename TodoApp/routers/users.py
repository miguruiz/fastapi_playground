...
class PasswordChangeRequest(BaseModel):
    ...
    new_pasword: str = Field(min_length=3)
    new_pasword_validation: str = Field(min_length=3)

    model_config = {
        "json_schema_extra": {
            "example": {
                "old_password": "old_passw",
                "new_pasword": "new_passw",
                "new_pasword_validation": "new_passw_again",
            }
        }
    }
...
async def change_password(
    ...
    if not passw_change.new_pasword == passw_change.new_pasword_validation:
        raise HTTPException(status_code=401, detail="New passwords don't match")

    user_model.hashed_password = bcrypt_context.hash(passw_change.new_pasword)
    db.add(user_model)
    db.commit()
